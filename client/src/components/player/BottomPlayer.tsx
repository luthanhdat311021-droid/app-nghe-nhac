...
export const BottomPlayer: React.FC = () => {
  ...
  return (
    <>
      ...
      {/* 📱 MOBILE MINI PLAYER (Rendered above BottomNavigation on < md screens) */}
      <div
        className="md:hidden fixed bottom-16 left-0 right-0 h-16 bg-[#0e0c18]/95 backdrop-blur-2xl border-t border-white/10 z-40 px-3 flex items-center justify-between shadow-2xl transition-all"
        style={{ marginBottom: 'env(safe-area-inset-bottom, 0px)' }}
        onClick={() => setIsFullScreenOpen(true)}
      >
        {/* Progress indicator bar at top of mini player */}
        ...
        <div className="flex items-center gap-3 min-w-0 flex-1 cursor-pointer">
          <div className="relative flex-shrink-0">
            <img
              src={currentSong.coverUrl || 'https://images.unsplash.com/photo-1614613535308-eb5fbd3d2c17?w=200&q=80'}
              alt={currentSong.title}
              className="w-11 h-11 rounded-lg object-cover border border-white/10"
            />
            {isYouTube && (
              <div className="absolute -top-1 -right-1 p-0.5 rounded-full bg-red-600 text-white shadow">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[11px] text-gray-400 truncate">{currentSong.artist?.name || 'Unknown Artist'}</p>
          </div>
        </div>

        {/* Right: Touch Controls (Favorite, Play/Pause, Next) */}
        <div className="flex items-center gap-1 flex-shrink-0" onClick={(e) => e.stopPropagation()}>
          <button
            onClick={handleFavoriteClick}
            className="p-2 text-gray-400 hover:text-white min-w-[40px] min-h-[40px] flex items-center justify-center"
            aria-label="Favorite song"
          >
            <Heart className={`w-4 h-4 ${currentSong.isFavorite ? 'text-pink-500 fill-pink-500' : ''}`} />
          </button>
          ...
          <button
            ...
            className="w-9 h-9 rounded-full bg-gradient-primary text-white flex items-center justify-center shadow-glow active:scale-95 transition-transform"
          ...
          <button
            ...
            className="p-2 text-gray-300 hover:text-white min-w-[40px] min-h-[40px] flex items-center justify-center active:scale-90"
      ...
      {/* 💻 DESKTOP PLAYER BAR (Rendered on md+ screens) */}
      <div className="hidden md:flex fixed bottom-0 left-0 right-0 h-24 bg-[#0e0c18]/95 backdrop-blur-2xl border-t border-white/10 z-40 px-6 items-center justify-between shadow-2xl transition-all">
